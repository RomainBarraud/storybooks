PROJECT_ID=high-office-343916
ZONE=us-central1-a

# export GOOGLE_APPLICATION_CREDENTIALS=$PWD/terraform-sa-key.json in terraform folder
...
terraform-action: check-env
	cd terraform && \
		terraform workspace select $(ENV) && \
		terraform $(TF_ACTION) \
		-var-file="./environments/common.tfvars" -var-file="./environments/$(ENV)/config.tfvars" \
...
SSH_STRING=romain2_barraud2@storybooks-vm-$(ENV)
...
deploy: check-env
	...
	@$(MAKE) ssh-cmd CMD='\
		docker run -d --name=$(CONTAINER_NAME) \
			...
			-p 80:3000 \
			-e PORT=3000 \
			-e \"MONGO_URI=mongodb+srv://storybooks-user-$(ENV):$(call get-secret,atlas_user_password_$(ENV))@storybooks-$(ENV).e1sfy.mongodb.net/$(DB_NAME)?retryWrites=true&w=majority\" \
